def pdfval(wTs,datai):
    ...
    sampdval = ((1/np.sqrt(2*math.pi))**(NN - m))*pref*((np.sqrt(2))**(NN - 2 - m))*sp.special.gamma((NN - m)/2)*(NN*msdatai - m*hbars)**((m - NN)/2)

    return (sampdval,h,hbars,evecs,evals)
